...
function ENT:Destruct()
    ...
    if IsValid(self:Getowning_ent()) then LynxonsRP.notify(self:Getowning_ent(), 1, 4, LynxonsRP.getPhrase("money_printer_exploded")) end
end

function ENT:BurstIntoFlames()
    if hook.Run("moneyPrinterCatchFire", self) == true then return end

    if IsValid(self:Getowning_ent()) then LynxonsRP.notify(self:Getowning_ent(), 0, 4, LynxonsRP.getPhrase("money_printer_overheating")) end
    self.burningup = true
...
function ENT:CreateMoneybag()
    ...
    local moneybag = LynxonsRP.createMoneyBag(MoneyPos, amount)
