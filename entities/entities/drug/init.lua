...
function ENT:Use(activator, caller)
    ...
    if activator ~= Owner then
        ...
        LynxonsRP.payPlayer(activator, Owner, self:Getprice())
        LynxonsRP.notify(activator, 0, 4, LynxonsRP.getPhrase("you_bought", LynxonsRP.getPhrase("drugs"), LynxonsRP.formatMoney(self:Getprice()), ""))
        LynxonsRP.notify(Owner, 0, 4, LynxonsRP.getPhrase("you_received_x", LynxonsRP.formatMoney(self:Getprice()), LynxonsRP.getPhrase("drugs")))
    end
    DrugPlayer(caller)
    self.CanUse = false
    self:Remove()
