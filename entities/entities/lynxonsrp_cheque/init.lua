...
function ENT:Use(activator, caller)
    ...
    if (IsValid(activator) and IsValid(recipient)) and activator == recipient then
        owner = (IsValid(owner) and owner:Nick()) or LynxonsRP.getPhrase("disconnected_player")
        LynxonsRP.notify(activator, 0, 4, LynxonsRP.getPhrase("found_cheque", LynxonsRP.formatMoney(amount), "", owner))
    ...
    elseif (IsValid(owner) and IsValid(recipient)) and owner ~= activator then
        LynxonsRP.notify(activator, 0, 4, LynxonsRP.getPhrase("cheque_details", recipient:Nick()))
    ...
    elseif IsValid(owner) and owner == activator then
        LynxonsRP.notify(activator, 0, 4, LynxonsRP.getPhrase("cheque_torn"))
...
function ENT:StartTouch(ent)
    ...
    if ent:GetClass() ~= "LynxonsRP_cheque" or self.USED or ent.USED or self.hasMerged or ent.hasMerged then return end
