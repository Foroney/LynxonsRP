...
local function createShipment(ply, args)
    ...
    if not shipID or ent.USED then
        LynxonsRP.notify(ply, 1, 4, LynxonsRP.getPhrase("unable", "/makeshipment", ""))
        return
    end
...
LynxonsRP.defineChatCommand("makeshipment", createShipment, 0.3)
...
local function splitShipment(ply, args)
    ...
    if ent:Getcount() < 2 or ent.locked or ent.USED then
        LynxonsRP.notify(ply, 1, 4, LynxonsRP.getPhrase("shipment_cannot_split"))
        return
    end
...
LynxonsRP.defineChatCommand("splitshipment", splitShipment, 0.3)
