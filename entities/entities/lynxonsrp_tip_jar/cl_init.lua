...
function ENT:Draw()
    ...
    owner = (IsValid(owner) and owner:Nick()) or LynxonsRP.getPhrase("unknown")
    local title = LynxonsRP.getPhrase("tip_jar")
...
function ENT:Donated(ply, amount)
    local txtAmount = LynxonsRP.formatMoney(amount)

    surface.SetFont("LynxonsRP_tipjar")
