...
SWEP.Spawnable = true
SWEP.AdminOnly = true
SWEP.Category = "LynxonsRP (Utility)"
...
function SWEP:PrimaryAttack()
    ...
    elseif ent:IsVehicle() then
        LynxonsRP.notify(Owner, 1, 3, LynxonsRP.getPhrase("do_not_own_ent"))
    else
...
function SWEP:Reload()
    ...
    if not IsValid(ent) or ((not ent:isDoor() and not ent:IsVehicle()) or self:GetOwner():EyePos():DistToSqr(trace.HitPos) > 40000) then
        if CLIENT and not LynxonsRP.disabledDefaults["modules"]["animations"] then RunConsoleCommand("_LynxonsRP_AnimationMenu") end
        return
    end
    if SERVER then
