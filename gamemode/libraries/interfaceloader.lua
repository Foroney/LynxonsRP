module("LynxonsRP", package.seeall)
...
local stubLayout = {
    ...
    metatable = istable -- LynxonsRP, Player, Entity, Vector, ...
}
...
function stub(tbl)
    ...
    if not isStub then
        error("Invalid LynxonsRP method stub! Field \"" .. field .. "\" is invalid!", 2)
    end
...
function hookStub(tbl)
    ...
    if not isStub then
        error("Invalid LynxonsRP hook! Field \"" .. field .. "\" is invalid!", 2)
    end
