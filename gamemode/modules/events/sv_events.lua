...
local function StormStart()
    local phrase = LynxonsRP.getPhrase("meteor_approaching")
...
local function StormEnd()
    local phrase = LynxonsRP.getPhrase("meteor_passing")
    for _, v in ipairs(player.GetAll()) do
        if v:Alive() then
            v:PrintMessage(HUD_PRINTCENTER, phrase)
            v:PrintMessage(HUD_PRINTTALK, phrase)
...
local function StartStorm(ply)
    ...
    LynxonsRP.notify(ply, 0, 4, LynxonsRP.getPhrase("meteor_enabled"))
end
LynxonsRP.definePrivilegedChatCommand("enablestorm", "LynxonsRP_AdminCommands", StartStorm)
...
local function StopStorm(ply)
    ...
    timer.Stop("start")
    StormEnd()
    LynxonsRP.notify(ply, 0, 4, LynxonsRP.getPhrase("meteor_disabled"))
end
LynxonsRP.definePrivilegedChatCommand("disablestorm", "LynxonsRP_AdminCommands", StopStorm)
...
hook.Add("PostCleanupMap", "LynxonsRP_events", createTremor)

hook.Add("InitPostEntity", "LynxonsRP_SetupTremor", function()
    createTremor()
end)
...
local function TremorReport()
    ...
    if mag then
        if mag < 6.5 then
            LynxonsRP.notifyAll(0, 3, LynxonsRP.getPhrase("earthtremor_report", tostring(mag)))
            return
        end
        LynxonsRP.notifyAll(0, 3, LynxonsRP.getPhrase("earthquake_report", tostring(mag)))
    end
end
