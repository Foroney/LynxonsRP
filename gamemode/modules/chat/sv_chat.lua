local function registerCommandDefinition(cmd, callback)
    local chatcommands = LynxonsRP.getChatCommands()
...
function LynxonsRP.defineChatCommand(cmd, callback)
...
function LynxonsRP.definePrivilegedChatCommand(cmd, priv, callback, extraInfoTbl)
    cmd = string.lower(cmd)

    local function onCAMIResult(ply, arg, hasAccess, reason)
        if hasAccess then return callback(ply, arg) end

        local notify = ply:EntIndex() == 0 and print or fp{LynxonsRP.notify, ply, 1, 4}
        notify(LynxonsRP.getPhrase("no_privilege"))
    end
...
local function RP_PlayerChat(ply, text, teamonly)
    LynxonsRP.log(ply:Nick() .. " (" .. ply:SteamID() .. "): " .. text)
    local callback = ""
    local DoSayFunc
    local groupSay = LynxonsRP.getChatCommand("g")

    -- Extract the chat command
    local tblCmd = fn.Compose{
        LynxonsRP.getChatCommand,
        string.lower,
    ...
    if string.sub(text, 1, 1) == GAMEMODE.Config.chatCommandPrefix and tblCmd then
        ...
        args = tblCmd.tableArgs and LynxonsRP.explodeArg(args) or args
...
local function RP_ActualDoSay(ply, text, callback)
    ...
    if GAMEMODE.Config.alltalk then
        ...
        for _, v in ipairs(player.GetAll()) do
            LynxonsRP.talkToPerson(v, col, callback .. name, col2, text, ply)
        end
    else
        LynxonsRP.talkToRange(ply, callback .. ply:Nick(), text, GAMEMODE.Config.talkDistance)
    end
    return ""
end

function GM:canChatCommand(ply, cmd, ...)
    if not ply.LynxonsRPUnInitialized then return true end

    LynxonsRP.notify(ply, 1, 4, LynxonsRP.getPhrase("data_not_loaded_one"))
    LynxonsRP.notify(ply, 1, 4, LynxonsRP.getPhrase("data_not_loaded_two"))
...
local function applyHookTable(tab, priority)
    ...
    if oldMetatable ~= nil then
        LynxonsRP.errorNoHalt("Some addon is fucking up LynxonsRP's chat hook reorganising mechanism. Start getting rid of scripts and addons until you don't see this error on startup anymore.")
        return
    end
...
function GM:PlayerSay(ply, text, teamonly) -- We will make the old hooks run AFTER LynxonsRP's playersay has been run.
...
-- LynxonsRP has local chat and all sorts of who-sees-what chat modifiers
-- In order for chat hooks to abide by those rules, we have to make sure that
-- the LynxonsRP chat hook (which is the gamemode function) runs first and last
-- All other chat hooks must be run within LynxonsRP's function for it to play
...
local function ReplaceChatHooks()
    ...
    fn.ForEach(warning, LynxonsRP.getIncompleteChatCommands())
...
local function ConCommand(ply, _, args)
    ...
    local tbl = LynxonsRP.getChatCommand(cmd)
...
concommand.Add("LynxonsRP", ConCommand)
