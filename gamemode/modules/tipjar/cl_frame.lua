...
function DONATE_ENTRY:CheckNumeric(value)
    if LynxonsRP.toInt(value) then return false end

    return true
end

function DONATE_ENTRY:OnChange()
    local value = LynxonsRP.toInt(self:GetText())
...
derma.DefineControl("LynxonsRP_TipJar_DONATE_ENTRY", "", DONATE_ENTRY, "DTextEntry")
...
function DONATE_BUTTON:Init()
    ...
    self.donateLabel:SetText(LynxonsRP.getPhrase("Donate"))
    ...
    onModelUpdate("amount", function(new)
        ...
        self.amountLabel:SetText(LynxonsRP.formatMoney(new))
...
derma.DefineControl("LynxonsRP_TipJar_DONATE_BUTTON", "", DONATE_BUTTON, "DButton")
...
function DONATE_LIST_ITEM:Init()
    ...
    self.textL:SetFont("LynxonsRPHUD2")
    self.textL:Dock(LEFT)

    self.textR = vgui.Create("DLabel", self)
    self.textR:SetFont("LynxonsRPHUD2")
...
derma.DefineControl("LynxonsRP_TipJar_DONATE_LIST_ITEM", "", DONATE_LIST_ITEM, "DPanel")
...
function DONATE_LIST:AddActiveLine(name, amount)
    local line = vgui.Create("LynxonsRP_TipJar_DONATE_LIST_ITEM", self)
...
function DONATE_LIST:RebuildLines(tipjar)
    ...
    for _, donation in ipairs(tipjar.madeDonations) do
        self:AddDonatedLine(donation.name, LynxonsRP.formatMoney(donation.amount))
    end
    ...
    for ply, amount in pairs(tipjar.activeDonations) do
        ...
        self.activeLines[ply:Nick()] = LynxonsRP.formatMoney(amount)
    end
...
derma.DefineControl("LynxonsRP_TipJar_DONATE_LIST", "", DONATE_LIST, "DPanelList")
...
function FRAME:Init()
    ...
    self.donateEntry = vgui.Create("LynxonsRP_TipJar_DONATE_ENTRY", self)
    self.donateEntry:SetPos(10, 175)

    self.donateButton = vgui.Create("LynxonsRP_TipJar_DONATE_BUTTON", self)
    self.donateButton:SetPos(10, 245)

    self.donateList = vgui.Create("LynxonsRP_TipJar_DONATE_LIST", self)
    self.donateList:SetPos(310, 25)


    self:SetSkin(GAMEMODE.Config.LynxonsRPSkin)
end
...
derma.DefineControl("LynxonsRP_TipJar_FRAME", "", FRAME, "DFrame")
...
onModelUpdate("frameVisible", function(visible)
    ...
    if not getModelValue("frame") then
        ...
        updateModel("frame", vgui.Create("LynxonsRP_TipJar_FRAME"))
    end
...
function LynxonsRP.tipJarUI(tipjar)
