...
local function updatePrivs()
    CAMI.PlayerHasAccess(LocalPlayer(), "LynxonsRP_ChangeDoorSettings", function(b, _)
        changeDoorAccess = b
    end)
end
...
function meta:drawOwnableInfo()
    ...
    if owned then
        table.insert(doorInfo, LynxonsRP.getPhrase("keys_owned_by"))
    end
    ...
    if playerOwned then
        ...
        if allowedCoOwn and not fn.Null(allowedCoOwn) then
            table.insert(doorInfo, LynxonsRP.getPhrase("keys_other_allowed"))
    ...
    elseif blocked and changeDoorAccess then
        table.insert(doorInfo, LynxonsRP.getPhrase("keys_allow_ownership"))
    elseif not blocked then
        table.insert(doorInfo, LynxonsRP.getPhrase("keys_unowned"))
        if changeDoorAccess then
            table.insert(doorInfo, LynxonsRP.getPhrase("keys_disallow_ownership"))
        end
    end
    ...
    if self:IsVehicle() then
        ...
        if driver:IsPlayer() then
            table.insert(doorInfo, LynxonsRP.getPhrase("driver", driver:Nick()))
        end
    end
...
Door data
---------------------------------------------------------------------------]]
LynxonsRP.doorData = LynxonsRP.doorData or {}

--[[---------------------------------------------------------------------------
Interface functions
---------------------------------------------------------------------------]]
function meta:getDoorData()
    local doorData = LynxonsRP.doorData[self:EntIndex()] or {}
...
local function retrieveAllDoorData(len)
    ...
    for i = 1, count do
        ...
        LynxonsRP.doorData[ix] = LynxonsRP.doorData[ix] or {}

        for vc = 1, varCount do
            local name, value = LynxonsRP.readNetDoorVar()
            LynxonsRP.doorData[ix][name] = value
        end
    end
end
net.Receive("LynxonsRP_AllDoorData", retrieveAllDoorData)
...
local function updateDoorData()
    ...
    LynxonsRP.doorData[door] = LynxonsRP.doorData[door] or {}

    local var, value = LynxonsRP.readNetDoorVar()

    LynxonsRP.doorData[door][var] = value
end
net.Receive("LynxonsRP_UpdateDoorData", updateDoorData)
...
local function removeDoorVar()
    ...
    local name = id == 0 and net.ReadString() or LynxonsRP.getDoorVars()[id].name

    if not LynxonsRP.doorData[door] then return end
    LynxonsRP.doorData[door][name] = nil
end
net.Receive("LynxonsRP_RemoveDoorVar", removeDoorVar)
...
local function removeDoorData()
    local door = net.ReadUInt(32)
    LynxonsRP.doorData[door] = nil
end
net.Receive("LynxonsRP_RemoveDoorData", removeDoorData)
