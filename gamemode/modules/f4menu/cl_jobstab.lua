...
function PANEL:setJob(job, closeFunc)
    ...
    elseif job.vote or job.RequiresVote and job.RequiresVote(LocalPlayer(), job.team) then
        ...
        self:SetText(LynxonsRP.getPhrase("create_vote_for_job"))
        self.DoClick = fn.Compose{closeFunc, fn.Partial(RunConsoleCommand, "LynxonsRP", "vote" .. job.command)}
    else
        self:SetVisible(true)
        self:SetText(LynxonsRP.getPhrase("become_job"))
        self.DoClick = fn.Compose{closeFunc, fn.Partial(RunConsoleCommand, "LynxonsRP", job.command)}
    end
end
...
function PANEL:OnMouseReleased()
    ...
    LynxonsRP.setPreferredJobModel(self.job.team, self.strModel)
end
...
function PANEL:updateInfo(job)
    ...
    local preferredModel = LynxonsRP.getPreferredJobModel(job.team)
...
function PANEL:Init()
    ...
    self.lblWeapons:SetText(LynxonsRP.getPhrase("F4guns"))
...
function PANEL:updateInfo(job)
    ...
    self.lblTitle:SetText(job.name and LynxonsRP.deLocalise(job.name) or (job.team and "" or "No jobs available"))
    ...
    if not job.weapons then
        ...
        weps = weps ~= "" and weps or LynxonsRP.getPhrase("no_extra_weapons")
    end
...
function PANEL:PerformLayout()
    local text = LynxonsRP.textWrap(LynxonsRP.deLocalise(self.job.description or ""):gsub('\t', ''), "Roboto Light", self:GetWide() - 43)
...
function PANEL:fillData()
    local categories = LynxonsRP.getCategories().jobs
    ...
    for _, cat in pairs(categories) do
        ...
        dCat:SetButtonFactory(function(item, ui)
            ...
            pnl.DoClick = fc{fp{self.pnlRight.updateInfo, self.pnlRight}, fp{fn.GetValue, "LynxonsRPItem", pnl}}
