...
local function ccDoVote(ply, cmd, args)
    ...
    if vote.voters[ply] or vote.exclude[ply] or canVote == false then
        LynxonsRP.notify(ply, 1, 4, message or LynxonsRP.getPhrase("you_cannot_vote"))
        return
    end
...
function Vote:handleEnd()
    ...
    Votes[self.id] = nil
    timer.Remove(self.id .. "LynxonsRPVote")
...
function LynxonsRP.hooks:canStartVote(newvote)
    if player.GetCount() <= table.Count(newvote.exclude) then
        ...
        if ply and ply:IsPlayer() then
            LynxonsRP.notify(ply, 0, 4, LynxonsRP.getPhrase("vote_alone"))
        end
        return false, true
    end
    return true
...
function LynxonsRP.createVote(question, voteType, target, time, callback, excludeVoters, fail, extraInfo)
    ...
    local canStart, callSuccess, message = hook.Call("canStartVote", LynxonsRP.hooks, newvote)
    if not canStart then
        if callSuccess then
            newvote:callback(1)
        else
            if ply:IsPlayer() then
                LynxonsRP.notify(ply, 1, 4, message or LynxonsRP.getPhrase("unable", LynxonsRP.getPhrase("create_vote_for_job"), ""))
            end
            newvote:fail()
        end
        return
    ...
    if ply:IsPlayer() then
        LynxonsRP.notify(ply, 0, 4, LynxonsRP.getPhrase("vote_started"))
    end
    ...
    timer.Create(newvote.id .. "LynxonsRPVote", time, 1, function() newvote:handleEnd() end)

    return newvote
end

function LynxonsRP.destroyVotesWithEnt(ent)
    for k, v in pairs(Votes) do
        if v.target ~= ent then continue end

        timer.Remove(v.id .. "LynxonsRPVote")
...
function LynxonsRP.destroyLastVote()
    local lastVote = Votes[#Votes]

    if not lastVote then return false end

    timer.Remove(lastVote.id .. "LynxonsRPVote")
...
local function CancelVote(ply)
    local result = LynxonsRP.destroyLastVote()

    if result then
        LynxonsRP.notifyAll(0, 4, LynxonsRP.getPhrase("x_cancelled_vote", ply:EntIndex() ~= 0 and ply:Nick() or "Console"))
        if ply:EntIndex() == 0 then
            print(LynxonsRP.getPhrase("x_cancelled_vote", "Console"))
        end
    else
        LynxonsRP.notify(ply, 1, 4, LynxonsRP.getPhrase("cant_cancel_vote"))
    end
end
LynxonsRP.definePrivilegedChatCommand("forcecancelvote", "LynxonsRP_AdminCommands", CancelVote)
