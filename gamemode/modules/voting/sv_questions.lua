...
function LynxonsRP.createQuestion(question, quesid, ent, delay, callback, fromPly, toPly, ...)
    local newques = {}
...
function LynxonsRP.destroyQuestion(id)
...
function LynxonsRP.destroyQuestionsWithEnt(ent)
    for _, v in pairs(Questions) do
        if v.Ent == ent then
            LynxonsRP.destroyQuestion(v.ID)
        end
    end
end
