...
local function HMHUD()
    local shouldDraw = hook.Call("HUDShouldDraw", GAMEMODE, "LynxonsRP_Hungermod")
    if shouldDraw == false then return end

    local energy = math.ceil(LocalPlayer():getLynxonsRPVar("Energy") or 0)
    ...
    if energy > 0 then
        ...
        draw.DrawNonParsedSimpleText(LynxonsRP.getPhrase("starving"), "ChatFont", HUDWidth / 2, y - 5, ConVars.StarvingText, 1)
    end
...
hook.Add("HUDDrawTargetID", "HMHUD", HMHUD) --HUDDrawTargetID is called after LynxonsRP HUD is drawn in HUDPaint
