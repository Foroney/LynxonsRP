...
function meta:newHungerData()
    self:setSelfLynxonsRPVar("Energy", 100)
end

function meta:hungerUpdate()
    if not GAMEMODE.Config.hungerspeed then return end

    local energy = self:getLynxonsRPVar("Energy")
    local override = hook.Call("hungerUpdate", nil, self, energy)

    if override then return end

    self:setSelfLynxonsRPVar("Energy", energy and math.Clamp(energy - GAMEMODE.Config.hungerspeed, 0, 100) or 100)

    if self:getLynxonsRPVar("Energy") == 0 then
