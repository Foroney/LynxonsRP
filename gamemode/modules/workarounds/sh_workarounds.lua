...
function LynxonsRP.getAvailableVehicles()
...
if not LynxonsRP.disabledDefaults["workarounds"]["os.date() Windows crash"] and system.IsWindows() then
...
timer.Simple(3, function()
    if LynxonsRP.disabledDefaults["workarounds"]["SkidCheck"] then return end
    ...
    SkidCheck would ban me (FPtje, developer of LynxonsRP) from your server because I have a
...
if SERVER and not LynxonsRP.disabledDefaults["workarounds"]["Error on edict limit"] then
    -- https://github.com/FPtje/LynxonsRP/issues/2640
    ...
    function ents.Create(name, ...)
        ...
        if res[1] == NULL and ents.GetEdictCount() >= 8176 then
            LynxonsRP.error(entsCreateError, 2, {string.format("Affected entity: '%s'", name)})
        end
...
---------------------------------------------------------------------------]]
hook.Add("InitPostEntity", "LynxonsRP_Workarounds", function()
    if CLIENT then
        if not LynxonsRP.disabledDefaults["workarounds"]["White flashbang flashes"] then
        ...
        -- Fuck up APAnti
        if not LynxonsRP.disabledDefaults["workarounds"]["APAnti"] then
    ...
    if not LynxonsRP.disabledDefaults["workarounds"]["Durgz witty sayings"] and commands["durgz_witty_sayings"] then
    ...
    if not LynxonsRP.disabledDefaults["workarounds"]["ULX /me command"] then
        hook.Remove("PlayerSay", "ULXMeCheck")
    end

    -- why can people even save multiplayer games?
    -- Lag exploit
    if not LynxonsRP.disabledDefaults["workarounds"]["gm_save"] then
        concommand.Remove("gm_save")
    end

    -- Remove that weird rooftop spawn in rp_downtown_v4c_v2
    if not LynxonsRP.disabledDefaults["workarounds"]["rp_downtown_v4c_v2 rooftop spawn"] and
...
---------------------------------------------------------------------------]]
if SERVER and not LynxonsRP.disabledDefaults["workarounds"]["Wire field generator exploit fix"] then
...
---------------------------------------------------------------------------]]
if not LynxonsRP.disabledDefaults["workarounds"]["Door tool class fix"] then
...
Anti crash exploit
---------------------------------------------------------------------------]]
if SERVER and not LynxonsRP.disabledDefaults["workarounds"]["Constraint crash exploit fix"] then
...
---------------------------------------------------------------------------]]
if SERVER and not LynxonsRP.disabledDefaults["workarounds"]["Deprecated console commands"] then
    local deprecated = {
    ...
    local function msgDeprecated(cmd, ply)
        ...
        LynxonsRP.notify(ply, 1, 4, ("This command has been deprecated. Please use 'LynxonsRP %s' or '/%s' instead."):format(cmd.alternative, cmd.alternative))
    end
...
addons\LynxonsRPmodification\lua\LynxonsRP_config\disabled_defaults.lua

Head to the section about disabled workarounds and make sure the following line is there:
["disable CAC"]                                  = true,
]]
if SERVER and not LynxonsRP.disabledDefaults["workarounds"]["disable CAC"] then
