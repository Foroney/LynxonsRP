...
function LynxonsRP.addLanguage(name, tbl)
    local old = rp_languages[name] or {}
    rp_languages[name] = tbl

    -- Merge the language with the translations added by LynxonsRP.addPhrase
...
function LynxonsRP.addPhrase(lang, name, phrase)
    rp_languages[lang] = rp_languages[lang] or {}
    rp_languages[lang][name] = phrase
end

function LynxonsRP.getPhrase(name, ...)
...
function LynxonsRP.getMissingPhrases(lang)
...
local function getMissingPhrases(ply, cmd, args)
    if not args[1] then print("Please run the command with a language code e.g. LynxonsRP_getphrases \"en\"") return end
    ...
    print(LynxonsRP.getMissingPhrases(args[1]))
end
if CLIENT then concommand.Add("LynxonsRP_getphrases", getMissingPhrases) end

--[[---------------------------------------------------------------------------
Chat command translating
---------------------------------------------------------------------------]]
local chatCmdDescriptions = {}
function LynxonsRP.addChatCommandsLanguage(lang, tbl)
    chatCmdDescriptions[lang] = chatCmdDescriptions[lang] or {}

    table.Merge(chatCmdDescriptions[lang], tbl)
end

function LynxonsRP.getChatCommandDescription(name)
    local cmd = LynxonsRP.getChatCommand(name)
...
local function getMissingCmdTranslations()
    local cmds = LynxonsRP.getSortedChatCommands()
...
if CLIENT then concommand.Add("LynxonsRP_translateChatCommands", printMissingChatTranslations) end
