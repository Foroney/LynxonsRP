...
function GM:CanChangeRPName(ply, RPname)
    if disallowedNames[string.lower(RPname)] then return false, LynxonsRP.getPhrase("forbidden_name") end
    if not string.match(RPname, "^[a-zA-ZЀ-џ0-9 ]+$") then return false, LynxonsRP.getPhrase("illegal_characters") end

    local len = string.len(RPname)
    if len > 30 then return false, LynxonsRP.getPhrase("too_long") end
    if len < 3 then return false,  LynxonsRP.getPhrase("too_short") end
end
...
function GM:LynxonsRPVarChanged(ply, var, oldvar, newvalue)
...
function GM:PlayerSpawnedProp(ply, model, ent)
    ...
    if GAMEMODE.Config.proppaying then
        if ply:canAfford(GAMEMODE.Config.propcost) then
            LynxonsRP.notify(ply, 0, 4, LynxonsRP.getPhrase("deducted_money", LynxonsRP.formatMoney(GAMEMODE.Config.propcost)))
            ply:addMoney(-GAMEMODE.Config.propcost)
        else
            LynxonsRP.notify(ply, 1, 4, LynxonsRP.getPhrase("need_money", LynxonsRP.formatMoney(GAMEMODE.Config.propcost)))
...
local function checkAdminSpawn(ply, configVar, errorStr)
    ...
    if (config == true or config == 1) and ply:EntIndex() ~= 0 and not ply:IsAdmin() then
        LynxonsRP.notify(ply, 1, 5, LynxonsRP.getPhrase("need_admin", LynxonsRP.getPhrase(errorStr) or errorStr))
        return false
    elseif config == 2 and ply:EntIndex() ~= 0 and not ply:IsSuperAdmin() then
        LynxonsRP.notify(ply, 1, 5, LynxonsRP.getPhrase("need_sadmin", LynxonsRP.getPhrase(errorStr) or errorStr))
        return false
    elseif config == 3 and ply:EntIndex() ~= 0 then
        LynxonsRP.notify(ply, 1, 5, LynxonsRP.getPhrase("disabled", LynxonsRP.getPhrase(errorStr) or errorStr, LynxonsRP.getPhrase("see_settings")))
        return false
    end
...
function GM:PlayerSpawnedSENT(ply, ent)
    ...
    LynxonsRP.log(ply:Nick() .. " (" .. ply:SteamID() .. ") spawned SENT " .. ent:GetClass(), Color(255, 255, 0))
end
...
local function canSpawnWeapon(ply)
    ...
    LynxonsRP.notify(ply, 1, 4, LynxonsRP.getPhrase("cant_spawn_weapons"))
...
function GM:PlayerSpawnedVehicle(ply, ent)
    ...
    LynxonsRP.log(ply:Nick() .. " (" .. ply:SteamID() .. ") spawned Vehicle " .. ent:GetClass() .. vehicleClass, Color(255, 255, 0))
end
...
function GM:PlayerSpawnedNPC(ply, ent)
    ...
    LynxonsRP.log(ply:Nick() .. " (" .. ply:SteamID() .. ") spawned NPC " .. ent:GetClass(), Color(255, 255, 0))
end
...
function GM:EntityRemoved(ent)
    ...
    if ent.LynxonsRPItem and IsValid(owner) and not ent.IsPocketing then owner:removeCustomEntity(ent.LynxonsRPItem) end
...
function GM:OnNPCKilled(victim, ent, weapon)
    ...
    if IsValid(ent) and hook.Call("canEarnNPCKillPay", GAMEMODE, ent, victim) then
        ...
        ent:addMoney(amount)
        LynxonsRP.notify(ent, 0, 4, LynxonsRP.getPhrase("npc_killpay", LynxonsRP.formatMoney(amount)))
    end
end
...
-- Set LynxonsRP.voiceCheckTimeDelay before LynxonsRP is loaded to set the time
-- between player voice radius checks.
LynxonsRP.voiceCheckTimeDelay = LynxonsRP.voiceCheckTimeDelay or 0.3
timer.Create("LynxonsRPCanHearPlayersVoice", LynxonsRP.voiceCheckTimeDelay, 0, function()
...
hook.Add("PlayerDisconnect", "LynxonsRPCanHear", function(ply)
...
function GM:CanPlayerSuicide(ply)
    if ply.IsSleeping then
        LynxonsRP.notify(ply, 1, 4, LynxonsRP.getPhrase("unable", "suicide", ""))
        return false
    end
    ...
    if GAMEMODE.Config.wantedsuicide and ply:getLynxonsRPVar("wanted") then
        LynxonsRP.notify(ply, 1, 4, LynxonsRP.getPhrase("unable", "suicide", ""))
        return false
    end
...
function GM:CanDrive(ply, ent)
    LynxonsRP.notify(ply, 1, 4, LynxonsRP.getPhrase("drive_disabled"))
...
function GM:CanProperty(ply, property, ent)
    ...
    LynxonsRP.notify(ply, 1, 4, LynxonsRP.getPhrase("property_disabled"))
...
function GM:PlayerDeath(ply, weapon, killer)
    ...
    if ply:isArrested() and not GAMEMODE.Config.respawninjail then
        ...
        LynxonsRP.printMessageAll(HUD_PRINTCENTER, LynxonsRP.getPhrase("died_in_jail", ply:Nick()))
        LynxonsRP.notify(ply, 4, 4, LynxonsRP.getPhrase("dead_in_jail"))
    else
    ...
    LynxonsRP.log(ply:Nick() .. " was killed by " .. KillerName .. " with a " .. WeaponName, Color(255, 190, 0))
end
...
function GM:PlayerCanPickupWeapon(ply, weapon)
    ...
    if GAMEMODE.Config.license and not ply:getLynxonsRPVar("HasGunlicense") and not ply.RPLicenseSpawn then
...
local function initPlayer(ply)
    timer.Simple(5, function()
        ...
        if GetGlobalBool("LynxonsRP_Lockdown") then
            SetGlobalBool("LynxonsRP_Lockdown", true) -- so new players who join know there's a lockdown, is this bug still there?
        end
    end)
    ...
    ply:setSelfLynxonsRPVar("salary", LynxonsRP.retrieveSalary(ply))
...
local function restoreReconnectedEnts(ply)
    ...
    timer.Remove("LynxonsRP_removeDisconnected_" .. sid)
    ...
    for _, e in pairs(queuedForRemoval[sid]) do
        ...
        -- Some entities (e.g. vehicles) have an SID, but do not have a LynxonsRPItem
        if e.LynxonsRPItem then
            ply:addCustomEntity(e.LynxonsRPItem)
        end
    end
...
function GM:PlayerInitialSpawn(ply)
    ...
    LynxonsRP.log(ply:Nick() .. " (" .. sid .. ") has joined the game", Color(0, 130, 255))
    ply:setLynxonsRPVarsAttribute()
...
function GM:PlayerSelectSpawn(ply)
    ...
    local CustomSpawnPos = LynxonsRP.retrieveTeamSpawnPos(ply:Team())
    ...
    if ply:isArrested() then
        POS = LynxonsRP.retrieveJailPos() or ply.DeathPos -- If we can't find a jail pos then we'll use where they died as a last resort
    end
    ...
    POS = LynxonsRP.findEmptyPos(POS, {ply}, 600, 30, hull)
...
function GM:PlayerSpawn(ply)
    if not ply.LynxonsRPInitialised then
        LynxonsRP.errorNoHalt(
            string.format("LynxonsRP was unable to introduce player \"%s\" to the game. Expect further errors and shit generally being fucked!",
            ...
            1,
            {
                ...
                "Errors in your LynxonsRP configuration (jobs, shipments, etc.) could also cause this. Earlier errors should tell you when this is the case."
            }
        )
    end
    ...
    if ply.demotedWhileDead then
        ...
        ply:setLynxonsRPVar("job", team.GetName(demoteTeam))
    end

    local jobTable = ply:getJobTable()

    player_manager.SetPlayerClass(ply, jobTable.playerClass or "player_LynxonsRP")
    ...
    LynxonsRP.log(ply:Nick() .. " (" .. ply:SteamID() .. ") spawned")
end
...
function GM:PlayerLoadout(ply)
    ...
    CAMI.PlayerHasAccess(ply, "LynxonsRP_GetAdminWeapons", function(access)
...
local function removeDelayed(entList, ply)
    ...
    timer.Create("LynxonsRP_removeDisconnected_" .. sid, removedelay, 1, function()
...
local function collectRemoveEntities(ply)
    ...
    local remClasses = {}
    for _, customEnt in pairs(LynxonsRPEntities) do
...
function GM:PlayerDisconnected(ply)
    ...
    LynxonsRP.destroyQuestionsWithEnt(ply)
    LynxonsRP.destroyVotesWithEnt(ply)

    if isMayor and GetGlobalBool("LynxonsRP_LockDown") then -- Stop the lockdown
        LynxonsRP.unLockdown(ply)
    end
    ...
    ply:keysUnOwnAll()
    LynxonsRP.log(ply:Nick() .. " (" .. ply:SteamID() .. ") disconnected", Color(0, 130, 255))
...
function GM:loadCustomLynxonsRPItems()
    ...
    if not GAMEMODE.DefaultTeam or not RPExtraTeams[GAMEMODE.DefaultTeam] then
        ...
        LynxonsRP.error("GAMEMODE.DefaultTeam is not set to an existing job.", 1, hints, "lua/LynxonsRP_customthings/jobs.lua", -1, stack)
    end
end
