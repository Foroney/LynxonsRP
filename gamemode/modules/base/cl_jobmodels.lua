...
Interface functions
---------------------------------------------------------------------------]]
function LynxonsRP.setPreferredJobModel(teamNr, model)
    ...
    net.Start("LynxonsRP_preferredjobmodel")
        net.WriteUInt(teamNr, 8)
        net.WriteString(model)
    net.SendToServer()
end

function LynxonsRP.getPreferredJobModel(teamNr)
    local job = RPExtraTeams[teamNr]
    if not job then return end
...
local function sendModels() -- run after the jobs have loaded
    net.Start("LynxonsRP_preferredjobmodels")
