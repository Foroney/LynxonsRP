...
function LynxonsRP.formatMoney(n)
...
---------------------------------------------------------------------------]]
function LynxonsRP.findPlayer(info)
...
Taken from FAdmin]]
---------------------------------------------------------------------------*/
function LynxonsRP.findPlayers(info)
...
local function GetAvailableVehicles(ply)
    ...
    print(LynxonsRP.getPhrase("rp_getvehicles") .. "\n")
...
--[[---------------------------------------------------------------------------
Whether a player has a LynxonsRP privilege
---------------------------------------------------------------------------]]
function meta:hasLynxonsRPPrivilege(priv)
    if FAdmin then
...
---------------------------------------------------------------------------]]
function LynxonsRP.nickSortedPlayers()
...
function LynxonsRP.explodeArg(arg)
    local args = {}
...
---------------------------------------------------------------------------]]
function LynxonsRP.ValidatedPhysicsInit(ent, solidType, hint)
    ...
    if solidType == SOLID_BSP then
        LynxonsRP.errorNoHalt(string.format("%s has no physics and will be motionless", class), 2, {
    ...
    if solidType == SOLID_VPHYSICS then
        ...
        if not mdl or mdl == "" then
            LynxonsRP.errorNoHalt(string.format("Cannot init physics on entity \"%s\" because it has no model", class), 2, {hint})
            return false
        end
        ...
        if util.IsValidProp(mdl) then
            ...
            LynxonsRP.errorNoHalt(string.format("physics limit hit - %s will be motionless", class), 2, {hint})

            return false
        end

        if not file.Exists(mdl, "GAME") then
            LynxonsRP.errorNoHalt(string.format("%s has missing model \"%s\" and will be invisible and motionless", class, mdl), 2, {
        ...
        LynxonsRP.errorNoHalt(string.format("%s has model \"%s\" with no physics and will be motionless", class, mdl), 2, {
    ...
    LynxonsRP.errorNoHalt(string.format("Unable to initilize physics on entity \"%s\"", class, {hint}), 2)
...
---------------------------------------------------------------------------]]
function LynxonsRP.toInt(value)
...
---------------------------------------------------------------------------]]
if SERVER then util.AddNetworkString("LynxonsRP_databaseCheckMessage") end
if CLIENT then net.Receive("LynxonsRP_databaseCheckMessage", fc{print, net.ReadString}) end
...
local function checkDatabase(ply)
    ...
    local display = (CLIENT or ply == game.GetWorld()) and print or function(msg)
            net.Start("LynxonsRP_databaseCheckMessage")
            net.WriteString(msg)
            net.Send(ply)
        end
    ...
    if MySQLite and MySQLite.isMySQL() then
        display(string.format([[WARNING: LynxonsRP is using MySQL. This only
...
concommand.Add("LynxonsRP_check_db_" .. (SERVER and "sv" or "cl"), checkDatabase)
